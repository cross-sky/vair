...
void Inp_adcDataProcess(void)
{
	...
	//2.采集完后是否已处理
	if (State_getDataProcessedFlag(adcstate) == STATE_DONE)
	{
		return;
	}
	//3.均值处理
...
void Inp_cd4051DataProcess(void)
{
	...
	//2.采集完后是否已处理
	if (State_getDataProcessedFlag(cd4051state) == STATE_DONE)
	{
		return;
	}
	//3.均值处理
	...
	//5.完成标志
	State_setDataProcessedFlag(cd4051state,STATE_DONE);
	//转换后设置到内存参数//need to define
